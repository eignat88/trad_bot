...
-- Shadow observability for MOMENTUM_EXHAUSTION_REVERSE_LONG_V2
...
SELECT
    ...
    SUM(total_scans) AS total_scans, SUM(pass_data_length) AS pass_data_length,
    SUM(pass_swing_highs) AS pass_swing_highs, SUM(pass_break_prev_high) AS pass_break_prev_high,
    SUM(pass_return_near_high) AS pass_return_near_high, SUM(pass_bearish_candle) AS pass_bearish_candle,
    SUM(pass_body_ratio) AS pass_body_ratio, SUM(pass_rsi_65) AS pass_rsi_65,
    ...
    SUM(no_data) AS no_data, SUM(no_swings) AS no_swings,
    SUM(no_breakout) AS no_breakout, SUM(too_far_above_prev_high) AS too_far_above_prev_high,
    SUM(not_bearish) AS not_bearish, SUM(body_too_large) AS body_too_large,
    SUM(rsi_below_65) AS rsi_below_65, SUM(rsi_delta_missing) AS rsi_delta_missing,
...
SELECT
    scanner_name, period_start, total_scans,
    pass_data_length, pass_swing_highs, pass_break_prev_high,
    pass_return_near_high, pass_bearish_candle, pass_body_ratio,
    pass_rsi_65, pass_rsi_delta_available, pass_rsi_delta_positive, final_setup,
    CASE WHEN total_scans > 0 THEN ROUND((pass_data_length::numeric / total_scans * 100), 2) ELSE 0 END AS pct_pass_data_length,
    CASE WHEN total_scans > 0 THEN ROUND((pass_swing_highs::numeric / total_scans * 100), 2) ELSE 0 END AS pct_pass_swing_highs,
    CASE WHEN total_scans > 0 THEN ROUND((pass_break_prev_high::numeric / total_scans * 100), 2) ELSE 0 END AS pct_pass_break_prev_high,
    CASE WHEN total_scans > 0 THEN ROUND((pass_return_near_high::numeric / total_scans * 100), 2) ELSE 0 END AS pct_pass_return_near_high,
    CASE WHEN total_scans > 0 THEN ROUND((pass_bearish_candle::numeric / total_scans * 100), 2) ELSE 0 END AS pct_pass_bearish_candle,
    CASE WHEN total_scans > 0 THEN ROUND((pass_body_ratio::numeric / total_scans * 100), 2) ELSE 0 END AS pct_pass_body_ratio,
    CASE WHEN total_scans > 0 THEN ROUND((pass_rsi_65::numeric / total_scans * 100), 2) ELSE 0 END AS pct_pass_rsi_65,
    CASE WHEN total_scans > 0 THEN ROUND((pass_rsi_delta_available::numeric / total_scans * 100), 2) ELSE 0 END AS pct_pass_rsi_delta_available,
    CASE WHEN total_scans > 0 THEN ROUND((pass_rsi_delta_positive::numeric / total_scans * 100), 2) ELSE 0 END AS pct_pass_rsi_delta_positive,
    CASE WHEN total_scans > 0 THEN ROUND((final_setup::numeric / total_scans * 100), 2) ELSE 0 END AS pct_final_setup
FROM dds.signal_funnel_observation
WHERE period_type = 'hourly'
...
SELECT
    ...
    SUM(rsi_below_65) AS rsi_below_65, SUM(rsi_delta_missing) AS rsi_delta_missing,
    SUM(rsi_delta_not_positive) AS rsi_delta_not_positive,
    SUM(total_scans) AS total_scans, SUM(final_setup) AS final_setup
FROM dds.signal_funnel_observation
WHERE period_type = 'hourly'
