...
class TestAnalyticsMigrations:
    ...
    def test_migration_creates_indexes(self, db_session):
        ...
        cursor.execute(
            "SELECT index_name FROM pg_indexes WHERE tablename = 'analysis_run' AND schemaname = 'analytics'"
        )
        ...
        cursor.execute(
            "SELECT index_name FROM pg_indexes WHERE tablename = 'candle' AND schemaname = 'market'"
        )
        indexes = cursor.fetchall()
