...
def uninstall_tasks() -> None:
    for task_name in (STOP_TASK_NAME, START_TASK_NAME):
...
def main() -> None:
    ...
    if args.action == "install":
        ...
        print(f"Installed tasks: {START_TASK_NAME} on startup, {STOP_TASK_NAME} weekdays at {STOP_TIME}")
    else:
        uninstall_tasks()
        print(f"Deleted tasks if present: {START_TASK_NAME}, {STOP_TASK_NAME}")
